 ...
 * Code Structure
 * ====================
 * The main XLIO Ultra API flow is located at the top of the file. Auxiliary code follows below.
 * Main parts:
 *  - XLIO Ultra API initialization and resource management
 *  - XLIO Ultra API TX path (xlio_socket_send)
 *  - XLIO Ultra API callbacks including RX path (socket_rx_cb)
 *  - Simple TX buffer pool implementation
 *  - RX data processing
 *  - Utility functions
 *
 * Application Protocol
 * ====================
 * The application implements a simple communication protocol: messages are separated by a
 * newline character ('\n'). Each message is a simple text string.
 */
...
struct app_config {
    bool is_server;
    const char *ip; /* Server IP: bind address (server) or destination (client) */
    const char *client_ip; /* Local IP to bind client socket to */
    unsigned short port;
    int ping_count;
...
/*
 * RX data accumulator for message assembly
 *
 * TCP doesn't preserve message boundaries, so we accumulate data until we find the message
 * delimiter (newline character).
 */
struct rx_accumulator {
    char *data; /* Accumulated data buffer */
    size_t len; /* Current length of accumulated data */
    size_t capacity; /* Allocated capacity of data buffer */
};

/* Application state */
struct app_state {
    struct xlio_api_t *api; /* XLIO Ultra API function pointers */
    xlio_poll_group_t group; /* XLIO polling group handle */
...
static struct app_config g_config = {
    .is_server = false,
    .ip = NULL, /* Server IP (required for client mode) */
    .client_ip = NULL, /* Optional: local IP to bind client socket */
    .port = 8080, /* Default TCP port */
    .ping_count = 3, /* Default number of ping messages to exchange */
 ...
 * These values are examples - adjust based on your application's:
 * - Message size requirements (TX_BUFFER_SIZE)
 * - Concurrent outstanding sends (TX_BUFFER_COUNT)
 */
#define TX_BUFFER_COUNT 128 /* Number of TX buffers in the pool */
#define TX_BUFFER_SIZE  1024 /* Size of each TX buffer in bytes */
#define TX_POOL_EMPTY   -1 /* Sentinel value indicating empty pool */

/* RX accumulator configuration */
#define RX_ACCUMULATOR_INITIAL_SIZE 1024 /* Initial size for RX accumulator buffer */

/*
 * Forward declarations
 */

/* Main XLIO usage flow */
...
/* Data processing */
static void process_rx_data(xlio_socket_t sock, void *data, size_t len);
static void process_received_message(xlio_socket_t sock, const char *msg);
...
int main(int argc, char **argv)
{
    ...
    /* XLIO Ultra API initialization. Must be done before other XLIO calls */
    ...
    /* Initialize TX buffer pool. Actual memory registration is done later on a connected socket */
    ...
    /* Cleanup application and XLIO resources */
    cleanup_resources();
...
static int init_xlio_ultra_api(void)
{
     ...
     * Uses xlio_get_api() which works when libxlio is preloaded with LD_PRELOAD.
     */
     ...
     * Checks that the loaded library supports XLIO Ultra API.
     */
    ...
    /*
     * Step 3: Initialize XLIO
     * Set up initialization attributes and call xlio_init_ex().
     */
    struct xlio_init_attr init_attr = {
        .flags = 0, /* Must be 0, reserved for future flags implementations */
        .memory_cb = NULL, /* Optional memory allocation callback */
        .memory_alloc = NULL, /* Optional external memory allocator function */
        .memory_free = NULL /* Can be optionally provided for external memory deallocation */
    };
...
static int create_polling_group(void)
{
    ...
    struct xlio_poll_group_attr group_attr = {
        /*
         * Bitwise-OR of the requested polling group flags:
         * - XLIO_GROUP_FLAG_SAFE:  Allow concurrent send and poll operations
         * - XLIO_GROUP_FLAG_DIRTY: Keep dirty sockets for batch flushing,
         *                          required for xlio_poll_group_flush()
         */
        .flags = 0,
 ...
 * Cleanup resources
 *
 * Destroy resources and exit XLIO.
 */
static void cleanup_resources(void)
{
    int rc;
    ...
    /* Cleanup RX accumulator */
    if (g_state.rx_acc.data) {
        free(g_state.rx_acc.data);
        g_state.rx_acc.data = NULL;
    }
 ...
 *  1. Gets a buffer from the TX buffer pool
 *  2. Copies the message
 *  3. Adds a newline delimiter according to our protocol
 *  4. Stores the buffer pointer in userdata_op for completion callback identification
 *  5. Sends using zero-copy interface
 */
static int send_message(xlio_socket_t sock, const char *msg)
{
    ...
    if (!g_state.mr_buf) {
        fprintf(stderr, "TX buffer memory not registered for zero-copy\n");
        return -1;
    }
    ...
    /* Get buffer from TX buffer pool */
    ...
    /* Construct application message with newline delimiter */
    ...
    rc = g_state.api->xlio_socket_send(sock, buffer, total_len, &send_attr);
    if (rc != 0) {
...
static int run_server_mode(void)
{
     ...
     * Note: socket_accept_cb must be registered in the polling group.
     */
...
static int run_client_mode(void)
{
     ...
     */
    if (g_config.client_ip) {
        memset(&client_addr, 0, sizeof(client_addr));
        client_addr.sin_family = AF_INET;
        client_addr.sin_port = 0; /* Let system choose port */

        rc = inet_aton(g_config.client_ip, &client_addr.sin_addr);
        if (rc == 0) {
            fprintf(stderr, "Invalid bind IP address: %s\n", g_config.client_ip);
            return -1;
        }

        rc = g_state.api->xlio_socket_bind(g_state.sock, (struct sockaddr *)&client_addr,
                                           sizeof(client_addr));
        if (rc != 0) {
...
static void socket_event_cb(xlio_socket_t sock, uintptr_t userdata_sq, int event, int value)
{
    ...
    case XLIO_SOCKET_EVENT_ESTABLISHED:
        ...
        if (userdata_sq == USERDATA_CLIENT) {
            ...
            /* This should not happen - server connections arrive via socket_accept_cb */
            fprintf(stderr,
...
static void socket_comp_cb(xlio_socket_t sock, uintptr_t userdata_sq, uintptr_t userdata_op)
{
    ...
    /*
     * In most cases, userdata_sq identifies the user's context related to the socket.
     * 'sock' argument can be used in simple cases to perform socket operations from the
     * callback without complex user's metadata.
     */
    (void)sock;
 ...
 *
 * Called when TCP payload arrives. Delegates all processing to process_rx_data().
 */
static void socket_rx_cb(xlio_socket_t sock, uintptr_t userdata_sq, void *data, size_t len,
                         struct xlio_buf *buf)
{
    printf("RX callback: received %zu bytes (socket userdata=%lx)\n", len, userdata_sq);

    /* Delegate to process_rx_data() which handles message parsing and dispatch */
    process_rx_data(sock, data, len);

    /* Return buffer to XLIO since data is processed */
...
static void socket_accept_cb(xlio_socket_t sock, xlio_socket_t parent, uintptr_t parent_userdata_sq)
{
    ...
    /*
     * 'parent' argument is designed for simple cases to perform parent socket operations
     * without associated user's metadata.
     */
    (void)parent;
 ...
 * RX data processing
 *
 * This function handles all the RX buffer processing logic including message reassembly.
 */
static void process_rx_data(xlio_socket_t sock, void *data, size_t len)
{
    struct rx_accumulator *acc = &g_state.rx_acc;
    size_t new_capacity;
    char *new_data;
    char *newline_pos;
    char *msg_start;
    ...
    /* Step 1: Allocate or grow accumulator buffer if needed */
    if (!acc->data || acc->len + len > acc->capacity) {
        new_capacity = max_size_t(acc->capacity * 2, RX_ACCUMULATOR_INITIAL_SIZE);
        /* Keep doubling until it fits */
        while (new_capacity < acc->len + len) {
            new_capacity *= 2;
        }

        /* Allocate or reallocate buffer (realloc handles NULL first argument) */
        new_data = realloc(acc->data, new_capacity);
        if (!new_data) {
            fprintf(stderr, "Failed to %s RX accumulator buffer to %zu bytes\n",
                    acc->data ? "grow" : "allocate", new_capacity);
            return;
        }
    ...
    /* Step 2: Append new data to accumulator */
    memcpy(acc->data + acc->len, data, len);
    acc->len += len;

    /* Step 3: Process all complete messages in the accumulator */
    msg_start = acc->data;
    while (acc->len > 0) {
        /* Search for the message delimiter in current accumulated data */
        newline_pos = memchr(msg_start, '\n', acc->len);
        if (!newline_pos) {
            break;
        }

        /* Found a complete message */
        msg_len = newline_pos - msg_start;
        /* Make a null-terminated message in-place */
        *newline_pos = '\0';

        printf("Received message: '%s' (%zu bytes)\n", msg_start, msg_len);

        /* Process the null-terminated message */
        process_received_message(sock, msg_start);

        /* Move to the next message */
        msg_start = newline_pos + 1;
        remaining = acc->len - (msg_start - acc->data);
        acc->len = remaining;
    }

    /* Step 4: Move any remaining incomplete message to start of buffer */
    if (acc->len > 0 && msg_start != acc->data) {
        memmove(acc->data, msg_start, acc->len);
    }
}
 ...
 *
 * The function is common for server and client and reacts to the message value:
 * - PING message triggers immediate PONG reply
 * - PONG message records statistics and triggers the next PING
 * - EXIT message makes the application exit
 */
static void process_received_message(xlio_socket_t sock, const char *msg)
{
    ...
    if (strcmp(msg, MSG_PING) == 0) {
        ...
        if (clock_gettime(CLOCK_MONOTONIC, &pong_received_time) == 0) {
            rtt_us = calculate_rtt_microseconds(&g_state.ping_sent_time, &pong_received_time);
 ...
 * ==========================================================================
 *
 * TX buffer pool is application-level logic, but is critical for performance.
 * An efficient TX buffer pool implementation can be complex and needs to take into
 * account hardware nuances, such as CPU caches, and application architecture (e.g.
 * multi-threading).
 *
 * Why Multiple TX Buffers Are Needed
 * ----------------------------------
 * Zero-copy send operations are asynchronous - a buffer cannot be reused immediately
 ...
 *    - Application constructs a message within the buffer
 *
 * 5. TRANSMISSION:
 ...
 *    - Ownership of the buffer is transferred back to the application
 *    -  XLIO no longer accesses the buffer after the completion event
 *
 * 7. RETURN TO POOL:
 ...
 * domain is unique per device (network interface). It can be determined for a socket after
 * the outgoing network device is defined for the socket. Therefore, protection domain can
...
static char *get_tx_buffer(void)
{
    ...
    if (g_state.pool_top == TX_POOL_EMPTY) {
        return NULL;
    }
...
static void cleanup_tx_buffer_pool(void)
{
    ...
    g_state.pool_top = TX_POOL_EMPTY;
}
...
/*
 * Return maximum of two size_t values
 *
 * Standard C has no max() for size_t, so we provide our own.
 */
static size_t max_size_t(size_t a, size_t b)
{
    return a > b ? a : b;
...
static int parse_command_line(int argc, char **argv)
{
    ...
    while ((option = getopt_long(argc, argv, "sci:p:I:n:t:h", long_options, &option_index)) != -1) {
        ...
        case 'p':
            ...
            g_config.port = (unsigned short)int_val;
            break;

        case 'I':
            g_config.client_ip = optarg;
            break;
     ...
     * Uses SIGALRM to terminate the application after specified seconds.
     * Useful for CI/CD pipelines and automated testing scenarios.
     */
    if (g_config.timeout_seconds > 0) {
 ...
 *
 * Computes the round-trip time between two timespec timestamps by converting
 * nanoseconds to microseconds for human-readable latency measurements.
 */
static double calculate_rtt_microseconds(const struct timespec *start, const struct timespec *end)
{
...
static void timeout_handler(int sig)
{
    /* Ignore the signal type - we register only SIGALRM */
    (void)sig;
