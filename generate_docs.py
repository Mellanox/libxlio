...
def _format_reference_body(properties: list[Property]) -> str:
    ...
    for prop in properties:
        ...
        parts.append(_format_property(prop))
...
def _format_property(prop: Property) -> str:
    ...
    parts.append(_format_type_line(prop.type_info))

    # Bold the env-var mention in the description text (handles both bare
    # names and names already wrapped in ** from the schema).
    desc = prop.description
    if prop.env_var:
        desc = _ENV_VAR_RE.sub(
            lambda m: f"Maps to **{m.group(1) or m.group(2)}** environment variable",
            desc,
        )
...
def main() -> None:
    ...
    body = _format_reference_body(properties)
...
def _cli() -> None:
    ...
    try:
        ...
        sys.exit(f"Error: unexpected schema structure: {exc}")
