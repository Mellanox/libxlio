...
if [ $opt_tarpkg -eq 1 -a "$rc" -eq 0 ]; then
    ...
    cp ${pkg_dir}/contrib/scripts/${pkg_spec} ${tarball_tmp}/${tarball_dir}/ >> ${pkg_log} 2>&1
    # Remove the duplicate spec file
    rm ${tarball_tmp}/${tarball_dir}/contrib/scripts/${pkg_spec} >> ${pkg_log} 2>&1
    
    # Create new tarball with proper structure
    pushd ${tarball_tmp} >> ${pkg_log} 2>&1
    tar -czf ${pkg_outdir}/${tarball_name} ${tarball_dir} >> ${pkg_log} 2>&1
    popd >> ${pkg_log} 2>&1
