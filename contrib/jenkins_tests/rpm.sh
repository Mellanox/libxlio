...
if [ $opt_binrpm -eq 1 ]; then
    if [ $opt_rpm -eq 1 ]; then
        test_exec="env RPM_BUILD_NCPUS=${NPROC} rpmbuild -bb $rpmmacros $rpmopts $rpmspec --define='\"configure_options $jenkins_test_custom_configure\"'"
    else
        test_exec="configure_options=\"$jenkins_test_custom_configure\" dpkg-buildpackage -us -uc -b"
    fi
