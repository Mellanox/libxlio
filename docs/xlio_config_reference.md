...
Distributes completion queue interrupts across CPU cores based on worker ID.

**Completion Vectors:** Each completion queue maps to an MSI-X interrupt line via a
completion vector index. The NIC's num_comp_vectors typically equals available CPU cores.

**Behavior:**
- Enabled (worker process): comp_vector = worker_id % num_comp_vectors
- Disabled: All completion queues use comp_vector=0 (all interrupts to CPU 0)

Only affects interrupt-driven I/O (epoll/blocking). No effect in pure polling mode.

**Tradeoffs:**
- *false* (default): Simple, predictable. Risk of CPU 0 hotspotting under high traffic
  (connections per second or requests per second) with multiple workers.
- *true*: Spreads interrupt load across cores. Better for multi-worker Nginx/Envoy with
  high connection or request rates.

**Auto-enabled** when nginx profile is active (workers_num > 0 or spec=nginx).

**Example:** 8 Nginx workers with 16 completion vectors: Workers 0-7 use comp_vectors 0-7,
spreading interrupts across 8 CPUs instead of concentrating all on CPU 0.
...
avoiding kernel file descriptor allocation and sockinfo object construction.

Only applies to worker processes (worker_id >= 0), not the NGINX master process.
...
Maps to **XLIO_NGINX_UDP_POOL_REUSE_BUFFS** environment variable.
...
Only applies when applications.nginx.udp_pool_size > 0.

**Value Tradeoffs:**

*0 (Default):*
Pooled sockets use rx_bufs_batch profile default (64 base, 8 with NGINX profile).
...
*Memory formula:*
Total pooled buffer memory ≈ udp_pool_size × this_value × buffer_size

*Example:*
udp_pool_size=50, udp_socket_pool_reuse=64, MTU=1500:
...
**Behavior:** When workers_num > 0, XLIO enables the nginx profile (ring per interface, TSO,
3-tuple rules), creates hardware flow steering rules per worker, and scales memory allocation.
...
*Non-power-of-2 values:* XLIO rounds up internally. Workers with ID < (workers_pow2 %
workers_num) receive secondary rules and handle more traffic slots.

**Memory:** 4GB per worker (workers_num ≤ 16) or 3GB per worker (workers_num > 16).
Master process uses minimal memory.

**Example:** 3 workers (rounds to workers_pow2=4): Worker 0 gets 2 port slots (primary +
...
> **Type:** integer or string — **Values:** -2/"exit", -1/"handle_debug", 0/"log_debug_undo_offload", 1/"log_error_undo_offload", 2/"log_error_return_error", 3/"log_error_abort"
...
Metadata stores buffer descriptors (mem_buf_desc_t) for tracking buffer state, ownership,
...
**Sizing:** Match to peak buffer demand. Each ring consumes:
- With Striding RQ (default): ~2GB (ring_elements_count × strides_num × stride_size)
- Without Striding RQ: ~32MB (ring_elements_count × rx_buf_size)

Total = per_ring_memory × number_of_rings. Rings depend on allocation_logic setting.

**Profile defaults:**
- ultra_latency: 128MB (single-ring focus)
- nginx: (workers_num > 16 ? 3GB : 4GB) × workers_num, divided per worker
- nginx_dpu: (workers_num == 16 ? 512MB : 1GB) × workers_num, divided per worker
- nvme_bf3: 256MB
...
- "SW RX Packets dropped:" non-zero → increase memory_limit or spare_buffers
...
**Auto-enabled** by profiles.spec=ultra_latency or profiles.spec=latency.
...
XLIO uses flow_tag = fd+1 for hardware flow steering. Without deferral:
1. Thread A calls close(fd=5), kernel releases fd immediately
2. Thread B creates socket, gets fd=5 (same number), same flow_tag=6
3. Hardware still has old flow steering rule (being destroyed by Thread A)
4. Packets for new socket may be misrouted or dropped
...
Memory formula: Work Queue Element buffer size = strides_num × stride_size.
...
With LRO disabled: Each packet uses ceil(packet_size / stride_size) strides.
- Waste per packet = stride_size - (packet_size mod stride_size)
...
Constraint: strides_num × stride_size must be at least MTU + 18 bytes (Ethernet headers).
...
**Memory:** Work Queue Element buffer = strides_num × stride_size.
Default: 2048 × 64 = 128KB per element. Total receive memory ≈ strides_num × stride_size × rx_num_wr.
...
**Constraint:** strides_num × rx_num_wr must not exceed 4,194,304 (hardware Completion Queue
Entry limit). If exceeded, rx_num_wr is automatically reduced.

**Default:** `2048`

### `hardware_features.tcp.lro`

> **Type:** integer or string — **Values:** -1/"auto", 0/"disable", 1/"enable"
...
Large Receive Offload max payload = min(strides_num × stride_size, 64KB), rounded to 256 bytes.
With defaults (2048 × 64 = 128KB), Large Receive Offload can coalesce full 64KB segments.
...
Only affects TLS TX offload (tx_enable). RX offload creates DEKs directly without caching.
...
Constraint: Must be < max_size. If >= max_size, auto-adjusted to max_size / 2.

Only affects TLS TX offload (tx_enable).
...
Formula: Actual payload = MIN(max_size, hardware_capability, congestion_window).
Also limited by tx_buf_size: actual_buf = MIN(tx_buf_size, max_size).
...
Prints resource usage report at process exit: buffer pools (created/free, memory,
expansions, unsatisfied requests), hugepages (allocation statistics), and hardware
receive drops.

Values:
- "auto" or -1 (default): Print only when anomalies detected. Anomalies include:
  - Unsatisfied buffer requests (memory pressure - suggests increasing core.memory_limit)
  - Hardware receive drops (packets dropped due to full queues)
  Recommended for production to surface resource issues without log noise.
- "disable" or 0: Never print. Use when exit logging is unwanted.
- "enable" or 1: Always print full report. Useful during development or profiling.

Note: Report requires graceful shutdown (SIGTERM). Will not appear if killed with SIGKILL.
...
> **Type:** integer or string — **Values:** -2/"init", -1/"none", 0/"panic", 1/"error", 2/"warn", 3/"info", 4/"details", 5/"debug", 6/"fine", 7/"finer", 8/"all"
...
- Deep troubleshooting: Use "fine" or "finer" with monitor.log.file_path to avoid
  console overhead
...
> **Type:** integer or string — **Values:** 0/"lwip", 1/"cubic", 2/"disable"
...
- *false*: Sends FIN, completes four-way handshake, enters TIME_WAIT (60-120 seconds).
...
- *false* (default): Reliable delivery, all pending data transmitted. TIME_WAIT consumes
  memory (~300-400 bytes per socket) and ephemeral ports. Keep for data integrity critical
...
How MSS is determined: Final MSS = MIN(configured_MSS if non-zero, interface_MTU - 40,
peer_advertised_MSS). The default (0) auto-calculates optimally from interface MTU.
...
- Path MTU Discovery broken (ICMP blocked) causing packet black-holing on paths with
  smaller MTU than your interface. Force smaller MSS as a workaround.
...
segment size (~1460 bytes) make nodelay largely ineffective.
...
acknowledged or unsent data reaches about one maximum segment size (~1460 bytes).
Fewer small packets; adds latency; can cause write-write-read deadlocks in some
protocols.
...
throughput; latency up to ~200 ms for small writes).
...
gaming, interactive); consider tcp.quickack for symmetric low latency.
...
Controls whether the TCP PUSH (PSH) flag is set on the last segment of each write.

**Behavior:** When true, XLIO sets PUSH on the last segment of every write; the
...
**TSO:** This setting does not prevent TSO from merging segments; TSO can batch
PSH-carrying segments with others, so transmission (and thus interactivity) may
be delayed until the TSO buffer is sent.
...
**Delayed ACKs (when quickack=false):** TCP batches ACKs, waiting up to timer_msec (default
100ms) before sending. Reduces ACK packet count by up to 50% and allows piggybacking ACKs
on response data.

**Nagle interaction:** When sender uses Nagle (nodelay=false) and receiver uses delayed ACKs,
both wait for each other, causing 40-200ms latency spikes. Enable quickack with nodelay for
symmetric low latency.

**Tradeoffs:**
- *false* (default): Efficient for bulk transfers, streaming, high-throughput. ACKs batched,
  lower packet rate and CPU. Adds up to 100ms ACK latency.
...
**Tradeoffs:**
- *Lower (10-50ms):* Faster packet loss detection, lower delayed ACK latency (improves sender's
  congestion window growth). Higher CPU overhead from more frequent timer processing. Better for
  latency-sensitive applications (trading, gaming).
- *Higher (200-500ms):* Lower CPU overhead, better for high connection counts. Slower loss
...
Reduce for latency-critical workloads. Increase for CPU-constrained servers with many connections.

**Constraints:**
- Minimum: performance.threading.internal_handler.timer_msec (auto-clamped if set lower)
...
> **Type:** integer or string — **Values:** 0/"disable", 1/"enable", 2/"os"
...
> **Type:** integer or string — **Values:** 0/"disable", 1/"raw_hw", 2/"best_possible", 3/"system", 4/"ptp", 5/"rtc"
...
> **Type:** integer or string — **Values:** 0/"disable", 1/"enable_and_reuse", 2/"enable"

Maps to **XLIO_BUFFER_BATCHING_MODE** environment variable.
Controls whether sockets cache buffers locally for reuse or return them immediately to the
shared pool. Batching amortizes lock overhead; reclaim prevents idle sockets from hoarding buffers.

**Values:**
- "disable" or 0: No caching. Every buffer operation acquires pool lock. Forces
  tcp_buffer_batch=1, udp_buffer_batch=1. Lowest memory, highest lock contention.
- "enable_and_reuse" or 1 (default): Caching with periodic reclaim. Every TCP timer tick
  (~100ms), idle cached buffers are returned to pool. Uses two-phase mechanism: first tick
...
- "enable" or 2: Caching without reclaim. Idle sockets keep buffers until destroyed. Lowest
  latency variance, but risk of buffer pool exhaustion if many sockets go idle.

**Threshold behavior (modes 1 and 2):** Buffers accumulate in socket's local cache until count
reaches threshold (~64), triggering batch return. At 2x threshold, forced immediate return.
...
**Buffer starvation risk (mode 2):** Without reclaim, idle sockets deplete the global pool.
New connections fail to allocate buffers; FIN packets cannot be processed, causing connection
leaks. Only use if all sockets remain active.
...
core.resources.memory_limit for memory sizing guidance.

**Auto-selected by:** nginx_dpu profile sets mode 0.

**Related:** network.protocols.tcp.timer_msec controls reclaim frequency in mode 1.
...
**With Striding Receive Queue enabled (default): no effect.** Buffer
size is determined by strides_num × stride_size. Keep at default (0).

**Without Striding Receive Queue:** Controls actual buffer allocation
size. Memory per ring ≈ buf_size × ring_elements_count × 2.
...
Match to MTU for standard use (auto does this). Higher values waste
memory on small packets; lower values risk truncation.

**When to change:** Only when Striding Receive Queue is disabled and
you need explicit control over buffer size. Otherwise keep default (0).
...
**Differs from prefetch_size:** prefetch_size prefetches the current
...
Match to what the CPU reads first when processing a packet: headers
(~40-60 bytes) plus start of payload. Cost: ceil(value / 64) prefetch
instructions per poll, paid even when no packet is waiting. 256 bytes
(4 instructions) covers typical headers + small payload with negligible
overhead.
...
ceil(value / 64) prefetch instructions starting after the Ethernet
header (~14 bytes into the buffer). The first ~30-40 prefetched bytes
are protocol headers (IP + TCP/UDP), not application payload.
Effective payload prefetch ≈ value - 40 bytes.

**Sizing:** Match to typical payload size + 40 (for headers). Default
(256) prefetches ~216 bytes of payload with 4 instructions - sufficient
for most workloads. Prefetching beyond packet size wastes instructions
...
**Related:** prefetch_before_poll prefetches the NEXT buffer BEFORE
...
Number of tcp_seg structures allocated from heap in a single expansion
when the global TCP segment pool is exhausted. Pool only grows; memory
is never returned until process exit.

**Caching hierarchy:** Sockets fetch from ring caches
(socket_batch_size, default 64), rings fetch from global pool
(ring_batch_size, default 1024), global pool expands by
pool_batch_size. Each tier reduces lock contention on the tier below.

**Memory per expansion:** pool_batch_size x 136 bytes
(default: 16384 x 136 = ~2.1MB).

**Sizing:** Each active TCP connection holds ~10-20 segments in send
and retransmit queues. Formula: peak_connections x 15 /
desired_expansions. Example: 1000 connections -> ~15000 segments ->
default 16384 covers it in one expansion. For most long-running
servers the default is sufficient.

**Tradeoffs:**
- *Higher:* Fewer expand() calls (each acquires global spinlock),
  lower latency variance during startup and traffic spikes. More
  memory upfront.
- *Lower:* Smaller footprint, gradual growth. More expand() calls
  under load cause brief latency hits from heap allocation and
  spinlock.

**Symptom of too-low value:** In xlio_stats, TCP_SEG_POOL section:
"No segments error:" non-zero or rising. With debug logging
(monitor.log.level=5), shutdown log shows "expands=N" count.
...
(socket_batch_size), rings from global pool (this parameter),
pool expands from heap (pool_batch_size). When a ring
accumulates > 2 x ring_batch_size segments, half are returned
(hysteresis).

**Memory:** Up to 2 x ring_batch_size x 136 bytes per ring
(default: ~272KB). Total = per-ring memory x ring count (set
by performance.rings.rx.allocation_logic).

**Sizing by ring allocation mode:**
- *per_interface* (1 ring): Default is more than sufficient.
- *per_thread* (default): Default works for most thread counts.
  Reduce to 512 for 32+ threads to limit total memory.
- *per_socket*: Use 256-512 to limit memory (1000 sockets x 2
  x 512 x 136 = ~139MB in segments alone).
...
Segments (136 bytes each) a socket fetches from its ring cache
...
**Sizing:** Each TCP connection holds ~10-20 segments in
send/retransmit queues. Memory per connection =
socket_batch_size x 136 bytes (default: ~8.5KB). Ring lock
acquired once per socket_batch_size sends. For 10K connections
at default: ~87MB in segment metadata alone.
...
**Ring allocation:** With per_thread (default), higher values
reduce lock contention between connections sharing a ring.
With per_socket, value mainly affects memory.
...
super-segment: tso.max_buf_sz = MIN(tx_buf_size, tso.max_size).
Without TCP Segmentation Offload, segments stay at Maximum Segment
Size regardless of buffer size.

Sizing: Match to typical send() payload size. For bulk transfers
(streaming, file serving), 64KB+ enables full hardware aggregation.
For small-message latency-sensitive apps, default (0) is sufficient.
...
Auto-modified by: worker_threads > 0 forces 256KB.
...
max_inline_size) bypass TX buffer copy, making prefetch
irrelevant.

How it works: Before copying into a TX buffer, XLIO issues
ceil(value / 64) prefetch instructions (one per cache line on
x86). Actual bytes prefetched = min(value, payload_size).
0 disables.

Sizing: Match to typical UDP payload size. Default (256) covers
headers plus ~200 bytes of payload with 4 instructions -
sufficient for most workloads. Increase for large-payload UDP
(jumbo frames). At millions of packets per second, reduce or
disable if profiling (perf stat -e cache-misses) shows prefetch
overhead.
...
are capped by adaptive_count and adaptive_period_usec.

**Value of 0:** Disables adaptive moderation entirely. Moderation uses
static packet_count and period_usec values only.

**Dependency:** Forced to 0 (disabled) if interrupt_moderation.enable
...
benefit; for short-lived connections, tune packet_count and
period_usec instead.
...
**Formula:** count = MIN(packet_rate / adaptive_interrupt_per_sec,
adaptive_count). Higher values allow more packets batched per
interrupt under heavy traffic.

**Hardware cap:** Also clamped at startup to half of total receive
completions: (strides_num × rx_num_wr) / 2 with Striding Receive
Queue (default), or rx_num_wr / 2 without.

**Sizing:** Set to peak_packet_rate / adaptive_interrupt_per_sec to
...
and adaptive_change_frequency_msec > 0.
...
**Formula:** The algorithm computes per ring each
adaptive_change_frequency_msec interval:
  count = MIN(packet_rate / this_value, adaptive_count)
  period = MIN(adaptive_period_usec,
               1000000/this_value - 1000000/packet_rate)
The NIC interrupts when count packets accumulate OR period elapses.
...
disable moderation entirely (interrupt_moderation.enable=false)
rather than setting a very high value here.

**Dependency:** Only effective when interrupt_moderation.enable is
true and adaptive_change_frequency_msec > 0.
...
**Formula:** The algorithm computes:
  period = 1000000/adaptive_interrupt_per_sec
           - 1000000/MAX(packet_rate, adaptive_interrupt_per_sec)
  actual_period = MIN(this_value, computed)
This parameter bounds worst-case latency. Under steady traffic,
...
on packet_count and period_usec thresholds rather than per-packet.

**Tradeoffs:**
- *true* (default): Hardware coalesces interrupts. Lower CPU usage at
  the cost of added latency (bounded by period_usec, default
  50 microseconds). Adaptive algorithm dynamically adjusts thresholds
  when adaptive_change_frequency_msec > 0.
...
blocking_rx_poll_usec=-1 for continuous polling, making interrupt-driven
notification redundant). Setting blocking_rx_poll_usec or
iomux.poll_usec to -1 manually has the same effect. Explicit user
...
hardware to this count + period_usec defaults).

**Trigger:** The NIC interrupts when EITHER this count is reached
OR period_usec expires, whichever comes first. Under heavy traffic,
count is usually reached first; under light traffic, period_usec
acts as the safety net.

**Maximum cap:** Clamped at startup to half of total receive
Completion Queue Entries: (strides_num x rx_num_wr) / 2 with
Striding Receive Queue (default), or rx_num_wr / 2 without.

**Sizing:** The adaptive algorithm (when
adaptive_change_frequency_msec > 0) overrides this value
...
light to reach packet_count, an interrupt fires within this period.

**Dual role:** Initial value when moderation is first enabled, and
fallback value when adaptive moderation detects zero traffic
(resets hardware to this period + packet_count defaults).

**Sizing:** Set to your maximum tolerable interrupt delay in
microseconds. Under heavy traffic, packet_count is usually reached
...
- *false*: Batched replenishment, lower CPU and jitter. Receive
  ...
  larger spare_buffers and rx_num_wr.

**Symptom of wrong setting:** Rising "SW RX Packets dropped:" in
xlio_stats means buffers are scarce - increase spare_buffers or
core.memory_limit before switching to false.
...
(controlled by performance.polling.max_rx_poll_batch, default 16).

**How it works:** Every periodic_drain_msec, the internal thread
locks each ring (non-blocking trylock) and drains up to this many
completions. The lock is held for the entire drain of each ring.

**Latency impact:** Lock hold time is approximately max_cqes
multiplied by 100-500 nanoseconds per completion. Default (10000)
can hold the lock 1-5 milliseconds under heavy load, blocking
application threads on the same ring. Reduce to bound jitter.
...
Each forced poll costs 50-200 nanoseconds. At millions of recv()
...
**Related:** skip_os (OS priority per N calls),
poll_usec (polling duration).
...
**Related:** blocking_rx_poll_usec (same for recv()/read()),
poll_os_ratio (OS polling within the busy-poll loop).
...
the OS is polled first; otherwise poll_os_ratio governs
...
**Related:** poll_os_ratio (Completion Queue-to-OS ratio
within polling loops), poll_usec (polling phase duration).
...
**Related:** blocking_rx_poll_usec (recv() poll duration),
hardware_features.lro (larger batches improve coalescing).
...
**Related:** ring_elements_count (transmit queue depth),
udp_buffer_batch (buffers fetched per send batch).
...
rx_wait() loops, sampling the OS socket (FIONREAD) and
polling any CQs that exist. When a ring attaches
(ADD_MEMBERSHIP or flow steering), blocking_rx_poll_usec
...
**Related:** blocking_rx_poll_usec (takes over post-ring
attach), yield_on_poll (CPU yielding during poll loop).
...
POSIX sockets; XLIO Ultra API sockets (entity_context)
poll independently and ignore this setting.
...
> **Type:** integer or string — **Values:** 0/"disable", 1/"enable", 2/"enable_epoll_only"
...
blocking_rx_poll_usec), letting other threads run when
...
- *Too high or 0:* Threads monopolize the core for the
  full blocking_rx_poll_usec budget, starving neighbors.

No effect when blocking_rx_poll_usec is 0 (no busy-poll
loop runs).
...
> **Type:** integer or string — **Values:** 0/"per_interface", 1/"per_ip_address", 10/"per_socket", 20/"per_thread", 30/"per_cpuid", 31/"per_core"
...
32+ threads, cap with max_per_interface (half the
...
internal_handler.behavior=delegate.
...
allocation_logic is per_thread, per_cpuid, or per_core.
...
**Related:** allocation_logic (ring assignment mode),
tx.migration_ratio (transmit side equivalent).
...
because each Work Queue Element holds strides_num (default
2048) packet slots — one doorbell already replenishes 2048
...
**Constraints:** ring_elements_count auto-raised to
2 x this value. spare_buffers auto-raised to this value.

**Related:** spare_buffers (buffer pool feeding batches),
ring_elements_count (Receive Queue depth).
...
element holds strides_num (default 2048) packet slots --
effective capacity = value x strides_num. Without Striding
Receive Queue, each element holds one packet.

**Memory per ring:** value x strides_num x stride_size with
Striding Receive Queue (128 x 2048 x 64 = 16 MB); value x
rx_buf_size without (32768 x 2048 = 64 MB).
...
**Constraints:** Auto-raised to 2 x post_batch_size. With
Striding Receive Queue, capped so strides_num x value
does not exceed 4,194,304 (maximum Completion Queue entries).

**Related:** post_batch_size (replenishment batch),
spare_buffers (defaults to half this value).
...
**Runtime default:** ring_elements_count / 2. Striding
Receive Queue: 64; without: 16384.

**Sizing:** Match to expected burst depth (packets arriving
faster than the application drains). Memory per ring =
value x buffer_size (~128 KB with Striding Receive Queue,
~MTU without).
...
**Related:** ring_elements_count (Receive Queue depth),
post_batch_size (replenishment batch), keep_full (drop
...
descriptors (~350 bytes) are metadata tracking packet
positions; they hold no packet data (spare_buffers
...
**Sizing:** Worst-case memory per ring = value x ~700
bytes (cache grows to 2x). Total = value x 700 x
ring count. Default with 32 rings = ~717 MB. To fit
a budget: value = budget / (700 x ring count), or
cap rings via max_per_interface.
...
**Related:** spare_buffers (data buffer cache),
striding_rq.enable.
...
**Sizing:** per_thread (20) suits most workloads. At
32+ threads, cap with max_per_interface (e.g. half
...
**Related:** max_per_interface (ring cap),
tx.migration_ratio (ring reassignment; disabled when
...
If ring_elements_count is below completion_batch_size × 2, XLIO
...
max_on_device_memory > 0): data on NIC internal memory, no PCIe
...
(max_on_device_memory) helps packets just above inline size.
...
the Work Queue Entry inline size (~204 bytes by default).
...
least completion_batch_size × 2. Memory per ring ≈ value
× 64 bytes.
...
xlio_stats -v 3 (full view) is non-zero. Total memory:
value × 64 × ring count × interface count (with per-thread
allocation, ring count equals thread count).
...
- *High (16, default):* Fewer pool lock acquisitions,
  ...
  by batching_mode periodic reclaim).
...
when worker_threads > 0 or batching_mode is "disable".
...
- *High (8, default):* Fewer pool lock acquisitions,
  ...
  batching_mode periodic reclaim).
...
Forced to 1 when batching_mode is "disable". For most
...
mc_flowtag_acceleration; when both are set,
mc_flowtag_acceleration is auto-disabled with a warning.
Multicast sockets with SO_REUSEADDR or SO_REUSEPORT
lose flow tags unless mc_flowtag_acceleration overrides.
tcp.2t_rules and tcp.3t_rules mask flow tags for
...
For servers (incoming accept()), prefer 3t_rules instead.
...
steering, see tcp.3t_rules (servers) and tcp.2t_rules
(clients).
...
every timer_msec (default 10 ms) via epoll_wait.
...
uses "0x01". Not supported in XLIO_INLINE_CONFIG.
...
internal thread. Unlike cpu_affinity (a scheduler hint),
...
aborts XLIO. When both cpuset and cpu_affinity are set,
the cpuset is joined first; cpu_affinity applies within
...
sufficient — use cpu_affinity alone for thread pinning.
...
> **Type:** integer or string — **Values:** 0/"disable", 1/"delegate"
...
**Constraint:** Lower bound for tcp.timer_msec — values
below this are raised automatically. Timer buckets =
tcp.timer_msec / timer_msec, so keep tcp.timer_msec an
integer multiple.
...
tcp.timer_msec (for example, 32 with tcp.timer_msec 256).
...
XLIO uses trylock (returns immediately if contended),
so the type matters mainly when contention forces a
blocking wait.
...
**Forced changes** when >0: buf_size 256 KB,
tcp_buffer_batch 1, poll_usec -1,
periodic_drain_msec 0.
...
> **Type:** integer or string — **Values:** 0/"none", 1/"ultra_latency", 2/"latency", 3/"nginx", 4/"nginx_dpu", 5/"nvme_bf3"
...
- *"nginx" (3):* Per-interface rings, TCP Segmentation
  ...
  applications.nginx.workers_num > 0 (must be set).
