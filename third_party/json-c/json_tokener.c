...
const char *doca_third_party_json_tokener_error_desc(enum json_tokener_error jerr)
{
...
enum json_tokener_error doca_third_party_json_tokener_get_error(struct json_tokener *tok)
{
	return tok->err;
}
...
struct json_tokener *doca_third_party_json_tokener_new_ex(int depth)
{
	...
	tok->pb = doca_third_party_printbuf_new();
	if (!tok->pb)
	{
		free(tok->stack);
		free(tok);
	...
	tok->max_depth = depth;
	doca_third_party_json_tokener_reset(tok);
	return tok;
}

struct json_tokener *doca_third_party_json_tokener_new(void)
{
	return doca_third_party_json_tokener_new_ex(JSON_TOKENER_DEFAULT_DEPTH);
}

void doca_third_party_json_tokener_free(struct json_tokener *tok)
{
	doca_third_party_json_tokener_reset(tok);
	if (tok->pb)
		doca_third_party_printbuf_free(tok->pb);
	free(tok->stack);
	free(tok);
}
...
static void json_tokener_reset_level(struct json_tokener *tok, int depth)
{
	...
	doca_third_party_json_object_put(tok->stack[depth].current);
	tok->stack[depth].current = NULL;
	free(tok->stack[depth].obj_field_name);
	tok->stack[depth].obj_field_name = NULL;
}

void doca_third_party_json_tokener_reset(struct json_tokener *tok)
{
	int i;
	if (!tok)
		return;
...
struct json_object *doca_third_party_json_tokener_parse(const char *str)
{
	enum json_tokener_error jerr_ignored;
	struct json_object *obj;
	obj = doca_third_party_json_tokener_parse_verbose(str, &jerr_ignored);
	return obj;
}

struct json_object *doca_third_party_json_tokener_parse_verbose(const char *str,
                                                                enum json_tokener_error *error)
{
	...
	if (!tok)
		return NULL;
	obj = doca_third_party_json_tokener_parse_ex(tok, str, -1);
	*error = tok->err;
...
#define printbuf_memappend_checked(p, s, l)                                 \
	do                                                                  \
	{                                                                   \
		if (doca_third_party_printbuf_memappend((p), (s), (l)) < 0) \
		{                                                           \
...
struct json_object *doca_third_party_json_tokener_parse_ex(struct json_tokener *tok,
                                                           const char *str, int len)
{
	...
	redo_char:
		...
		case json_tokener_state_eatws:
			...
			{
				doca_third_party_printbuf_reset(tok->pb);
		...
		case json_tokener_state_start:
			...
			case '{':
				...
				current = doca_third_party_json_object_new_object();
				if (current == NULL)
				{
					tok->err = json_tokener_error_memory;
					goto out;
			...
			case '[':
				...
				current = doca_third_party_json_object_new_array();
				if (current == NULL)
				{
					tok->err = json_tokener_error_memory;
					goto out;
		...
		case json_tokener_state_finish:
			if (tok->depth == 0)
				goto out;
			obj = doca_third_party_json_object_get(current);
		...
		case json_tokener_state_inf: /* aka starts with 'i' (or 'I', or "-i", or "-I") */
		{
			...
			current = doca_third_party_json_object_new_double(is_negative ? -INFINITY
			                                                              : INFINITY);
			if (current == NULL)
			{
				tok->err = json_tokener_error_memory;
		...
		case json_tokener_state_null: /* aka starts with 'n' */
		{
			         ...
			         (strncmp(json_nan_str, tok->pb->buf, size_nan) == 0))
			{
				...
				{
					current = doca_third_party_json_object_new_double(NAN);
					if (current == NULL)
					{
						tok->err = json_tokener_error_memory;
						goto out;
		...
		case json_tokener_state_string:
		{
			...
			while (1)
			{
				if (c == tok->quote_char)
				{
					printbuf_memappend_checked(tok->pb, case_start,
					                           str - case_start);
					current = doca_third_party_json_object_new_string_len(
					    tok->pb->buf, tok->pb->bpos);
					if (current == NULL)
					{
						tok->err = json_tokener_error_memory;
						goto out;
		...
		case json_tokener_state_escape_unicode:
		{
			...
			else if (IS_LOW_SURROGATE(tok->ucs_char))
			{
				...
				printbuf_memappend_checked(tok->pb, (char *)utf8_replacement_char,
				                           3);
			}
		...
		case json_tokener_state_boolean:
		{
			    ...
			    (strncmp(json_true_str, tok->pb->buf, size1) == 0))
			{
				...
				{
					current = doca_third_party_json_object_new_boolean(1);
					if (current == NULL)
					{
						tok->err = json_tokener_error_memory;
						goto out;
			         ...
			         (strncmp(json_false_str, tok->pb->buf, size2) == 0))
			{
				...
				{
					current = doca_third_party_json_object_new_boolean(0);
					if (current == NULL)
					{
						tok->err = json_tokener_error_memory;
						goto out;
		...
		case json_tokener_state_number:
		{
		...
		}
			{
				...
				if (!tok->is_double && tok->pb->buf[0] == '-' &&
				    doca_third_party_json_parse_int64(tok->pb->buf, &num64) == 0)
				{
					...
					current = doca_third_party_json_object_new_int64(num64);
					if (current == NULL)
					{
						tok->err = json_tokener_error_memory;
						goto out;
				         ...
				         doca_third_party_json_parse_uint64(tok->pb->buf,
				                                            &numuint64) == 0)
				{
					if (errno == ERANGE && (tok->flags & JSON_TOKENER_STRICT))
					{
					...
					if (numuint64 <= INT64_MAX)
					{
						...
						current =
						    doca_third_party_json_object_new_int64(num64);
						if (current == NULL)
						{
							tok->err = json_tokener_error_memory;
							goto out;
					...
					{
						current = doca_third_party_json_object_new_uint64(
						    numuint64);
						if (current == NULL)
						{
							tok->err = json_tokener_error_memory;
				...
				{
					current = doca_third_party_json_object_new_double_s(
					    numd, tok->pb->buf);
					if (current == NULL)
					{
						tok->err = json_tokener_error_memory;
		...
		case json_tokener_state_array:
			if (c == ']')
			{
				...
				doca_third_party_json_object_array_shrink(current, 0);
		...
		case json_tokener_state_array_add:
			if (doca_third_party_json_object_array_add(current, obj) != 0)
			{
				tok->err = json_tokener_error_memory;
				goto out;
			}
		...
		case json_tokener_state_object_value_add:
			doca_third_party_json_object_object_add(current, obj_field_name, obj);
			free(obj_field_name);
			obj_field_name = NULL;
...
out:
	...
	if (tok->err == json_tokener_success)
	{
		json_object *ret = doca_third_party_json_object_get(current);
		int ii;
...
void doca_third_party_json_tokener_set_flags(struct json_tokener *tok, int flags)
{
	tok->flags = flags;
}

size_t doca_third_party_json_tokener_get_parse_end(struct json_tokener *tok)
{
