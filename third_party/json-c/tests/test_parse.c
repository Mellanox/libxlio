...
static void single_incremental_parse(const char *test_string, int clear_serializer)
{
	...
	new_obj = NULL;
	assert(chunksize > 0);
	all_at_once_obj = doca_third_party_json_tokener_parse(test_string);
	if (clear_serializer)
		do_clear_serializer(all_at_once_obj);
	all_at_once_str = doca_third_party_json_object_to_json_string(all_at_once_obj);
	...
	for (ii = 0; ii < test_string_len; ii += chunksize)
	{
		...
		new_obj = doca_third_party_json_tokener_parse_ex(tok, &test_string[ii], len_to_parse);
		jerr = doca_third_party_json_tokener_get_error(tok);
		if (jerr != json_tokener_continue || new_obj)
			break;
	}
	if (clear_serializer && new_obj)
		do_clear_serializer(new_obj);
	new_str = doca_third_party_json_object_to_json_string(new_obj);
	...
	if (all_at_once_obj)
		doca_third_party_json_object_put(all_at_once_obj);
	if (new_obj)
...
static void single_basic_parse(const char *test_string, int clear_serializer)
{
	...
	new_obj = doca_third_party_json_tokener_parse(test_string);
	if (clear_serializer)
		do_clear_serializer(new_obj);
	printf("new_obj.to_string(%s)=%s\n", test_string, doca_third_party_json_object_to_json_string(new_obj));
	doca_third_party_json_object_put(new_obj);
...
static void do_clear_serializer(json_object *jso)
{
	doca_third_party_json_c_visit(jso, 0, clear_serializer, NULL);
}
...
static void test_verbose_parse(void)
{
	...
	new_obj = doca_third_party_json_tokener_parse_verbose("{ foo }", &error);
	assert(error == json_tokener_error_parse_object_key_name);
	assert(new_obj == NULL);

	new_obj = doca_third_party_json_tokener_parse("{ foo }");
	assert(new_obj == NULL);
...
static void test_incremental_parse(void)
{
	...
	new_obj = doca_third_party_json_tokener_parse(string_to_parse);
	if (new_obj == NULL)
	...
	for (ii = 0; incremental_steps[ii].string_to_parse != NULL; ii++)
	{
		...
		doca_third_party_json_tokener_set_flags(tok, step->tok_flags);
		...
		printf("json_tokener_parse_ex(tok, %-12s, %3d) ... ", step->string_to_parse,
		       length);
		new_obj = doca_third_party_json_tokener_parse_ex(tok, step->string_to_parse, length);
		...
		if (step->expected_error != json_tokener_success)
		{
			if (new_obj != NULL)
				...
				printf("ERROR: got wrong error: %s\n",
				       doca_third_party_json_tokener_error_desc(jerr));
			else if (doca_third_party_json_tokener_get_parse_end(tok) != expected_char_offset)
				printf("ERROR: wrong char_offset %zu != expected %zu\n",
				       doca_third_party_json_tokener_get_parse_end(tok), expected_char_offset);
			else
			{
		...
		else
		{
			...
			else
			{
				printf("OK: got object of type [%s]: %s\n",
				       doca_third_party_json_type_to_name(doca_third_party_json_object_get_type(new_obj)),
				       doca_third_party_json_object_to_json_string(new_obj));
				this_step_ok = 1;
			}
		}
