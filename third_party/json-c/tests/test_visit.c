...
int main(void)
{
	...
	json_object *jso = doca_third_party_json_tokener_parse(input);
	printf("jso.to_string()=%s\n", doca_third_party_json_object_to_json_string(jso));

	int rv;
	rv = doca_third_party_json_c_visit(jso, 0, emit_object, NULL);
	printf("json_c_visit(emit_object)=%d\n", rv);
	printf("================================\n\n");

	rv = doca_third_party_json_c_visit(jso, 0, skip_arrays, NULL);
	printf("json_c_visit(skip_arrays)=%d\n", rv);
	printf("================================\n\n");

	rv = doca_third_party_json_c_visit(jso, 0, pop_and_stop, NULL);
	printf("json_c_visit(pop_and_stop)=%d\n", rv);
	printf("================================\n\n");

	rv = doca_third_party_json_c_visit(jso, 0, err_on_subobj2, NULL);
	printf("json_c_visit(err_on_subobj2)=%d\n", rv);
	printf("================================\n\n");

	rv = doca_third_party_json_c_visit(jso, 0, pop_array, NULL);
	printf("json_c_visit(pop_array)=%d\n", rv);
	printf("================================\n\n");

	rv = doca_third_party_json_c_visit(jso, 0, stop_array, NULL);
	printf("json_c_visit(stop_array)=%d\n", rv);
	printf("================================\n\n");

	rv = doca_third_party_json_c_visit(jso, 0, err_return, NULL);
                       ...
                       size_t *jso_index, void *userarg)
{
	...
	if (doca_third_party_json_object_get_type(jso) == json_type_array)
