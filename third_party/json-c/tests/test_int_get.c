...
#define CHECK_GET(GET_F, J, EXPECTED)     { struct json_object *jtmp = J; errno = 0; assert(GET_F(jtmp) == EXPECTED); doca_third_party_json_object_put(jtmp); }
#define CHECK_GET_INT(J, EXPECTED)        CHECK_GET( doca_third_party_json_object_get_int,    J, EXPECTED)
#define CHECK_GET_INT64(J, EXPECTED)      CHECK_GET( doca_third_party_json_object_get_int64,  J, EXPECTED)
#define CHECK_GET_UINT64(J, EXPECTED)     CHECK_GET( doca_third_party_json_object_get_uint64, J, EXPECTED)

#define CHECK_BASE(J, EXPECTED)						CHECK_GET_INT(J, EXPECTED); CHECK_GET_INT64(J, EXPECTED); CHECK_GET_UINT64(J, EXPECTED)

#define N_INT     doca_third_party_json_object_new_int
#define N_I64     doca_third_party_json_object_new_int64
#define N_U64     doca_third_party_json_object_new_uint64
#define N_STR     doca_third_party_json_object_new_string
