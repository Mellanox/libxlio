...
{
	const char *comment = doca_third_party_json_object_get_string(
	    doca_third_party_json_object_object_get(jo, "comment"));
	struct json_object *doc = doca_third_party_json_object_object_get(jo, "doc");
	struct json_object *patch = doca_third_party_json_object_object_get(jo, "patch");
	struct json_object *expected = NULL;
	json_bool have_expected =
	    doca_third_party_json_object_object_get_ex(jo, "expected", &expected);
	struct json_object *error = doca_third_party_json_object_object_get(jo, "error");
	const char *error_s = doca_third_party_json_object_get_string(error);
	struct json_object *res = NULL;
	int ret;

	printf("Testing '%s', doc '%s' patch '%s' : ", comment ? comment : error_s,
	       doca_third_party_json_object_get_string(doc),
	       doca_third_party_json_object_get_string(patch));
	if (!error && !have_expected)
	{
		printf("BAD TEST - no expected or error conditions in test: %s\n",
		       doca_third_party_json_object_to_json_string(jo));
		assert(0);
	}
	fflush(stdout);
	...
	if (error)
	{
		assert(-1 == doca_third_party_json_patch_apply(doc, patch, &res, &jperr));
		...
		doca_third_party_json_object_put(res);
	}
	else
	{
		ret = doca_third_party_json_patch_apply(doc, patch, &res, &jperr);
		if (ret)
		{
			fprintf(stderr, "json_patch_apply() returned '%d'\n", ret);
			fprintf(stderr, "Expected: %s\n",
			        doca_third_party_json_object_get_string(expected));
			fprintf(stderr, "Got: %s\n",
			        res ? doca_third_party_json_object_get_string(res) : "(null)");
		...
		assert(jperr.errno_code == 0);
		ret = doca_third_party_json_object_equal(expected, res);
		if (ret == 0)
		{
			fprintf(stderr, "json_object_equal() returned '%d'\n", ret);
			fprintf(stderr, "Expected: %s\n",
			        doca_third_party_json_object_get_string(expected));
			fprintf(stderr, "Got: %s\n", doca_third_party_json_object_get_string(res));
			fflush(stderr);
			assert(0);
		}
		doca_third_party_json_object_put(res);
...
void test_json_patch_using_file(const char *testdir, const char *filename)
{
	...
	json_object *jo = doca_third_party_json_object_from_file(full_filename);
	if (!jo)
	{
	...
	for (ii = 0; ii < doca_third_party_json_object_array_length(jo); ii++)
	{
		struct json_object *jo1 = doca_third_party_json_object_array_get_idx(jo, ii);
		test_json_patch_op(jo1);
	}

	doca_third_party_json_object_put(jo);
}
