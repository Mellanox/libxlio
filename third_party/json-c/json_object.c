...
const char *doca_third_party_json_number_chars =
    "0123456789.+-eE"; /* Unused, but part of public API, drop for 1.0 */
const char *doca_third_party_json_hex_chars = "0123456789abcdefABCDEF";
...
static int json_escape_str(struct printbuf *pb, const char *str, size_t len, int flags)
{
	...
	while (len)
	{
		...
		case '/':
			...
			if (pos > start_offset)
				doca_third_party_printbuf_memappend(pb, str + start_offset, pos - start_offset);

			if (c == '\b')
				doca_third_party_printbuf_memappend(pb, "\\b", 2);
			else if (c == '\n')
				doca_third_party_printbuf_memappend(pb, "\\n", 2);
			else if (c == '\r')
				doca_third_party_printbuf_memappend(pb, "\\r", 2);
			else if (c == '\t')
				doca_third_party_printbuf_memappend(pb, "\\t", 2);
			else if (c == '\f')
				doca_third_party_printbuf_memappend(pb, "\\f", 2);
			else if (c == '"')
				doca_third_party_printbuf_memappend(pb, "\\\"", 2);
			else if (c == '\\')
				doca_third_party_printbuf_memappend(pb, "\\\\", 2);
		...
		default:
			if (c < ' ')
			{
				char sbuf[7];
				if (pos > start_offset)
					doca_third_party_printbuf_memappend(pb, str + start_offset,
					                   pos - start_offset);
				snprintf(sbuf, sizeof(sbuf), "\\u00%c%c", doca_third_party_json_hex_chars[c >> 4],
				         doca_third_party_json_hex_chars[c & 0xf]);
...
struct json_object *doca_third_party_json_object_get(struct json_object *jso)
{
	if (!jso)
		return jso;
...
int doca_third_party_json_object_put(struct json_object *jso)
{
	if (!jso)
		return 0;
...
static void json_object_generic_delete(struct json_object *jso)
{
	doca_third_party_printbuf_free(jso->_pb);
	free(jso);
}
...
int doca_third_party_json_object_is_type(const struct json_object *jso, enum json_type type)
{
	if (!jso)
...
enum json_type doca_third_party_json_object_get_type(const struct json_object *jso)
{
	if (!jso)
...
void *doca_third_party_json_object_get_userdata(json_object *jso)
{
	return jso ? jso->_userdata : NULL;
}

void doca_third_party_json_object_set_userdata(json_object *jso, void *userdata, json_object_delete_fn *user_delete)
{
...
void doca_third_party_json_object_set_serializer(json_object *jso, json_object_to_json_string_fn *to_string_func,
                                void *userdata, json_object_delete_fn *user_delete)
{
	doca_third_party_json_object_set_userdata(jso, userdata, user_delete);
...
const char *doca_third_party_json_object_to_json_string_length(struct json_object *jso, int flags, size_t *length)
{
	const char *r = NULL;
	size_t s = 0;
	...
	else if ((jso->_pb) || (jso->_pb = doca_third_party_printbuf_new()))
	{
		doca_third_party_printbuf_reset(jso->_pb);
...
const char *doca_third_party_json_object_to_json_string_ext(struct json_object *jso, int flags)
{
	return doca_third_party_json_object_to_json_string_length(jso, flags, NULL);
}

/* backwards-compatible conversion to string */

const char *doca_third_party_json_object_to_json_string(struct json_object *jso)
{
	return doca_third_party_json_object_to_json_string_ext(jso, JSON_C_TO_STRING_SPACED);
}
...
static void indent(struct printbuf *pb, int level, int flags)
{
	if (flags & JSON_C_TO_STRING_PRETTY)
	{
		...
		{
			doca_third_party_printbuf_memset(pb, -1, '\t', level);
		}
		else
		{
                                             ...
                                             int level, int flags)
{
	...
	doca_third_party_json_object_object_foreachC(jso, iter)
	{
		if (had_children)
		{
			printbuf_strappend(pb, ",");
...
static void json_object_lh_entry_free(struct lh_entry *ent)
{
	if (!lh_entry_k_is_constant(ent))
		free(lh_entry_k(ent));
	doca_third_party_json_object_put((struct json_object *)lh_entry_v(ent));
}

static void json_object_object_delete(struct json_object *jso_base)
{
	doca_third_party_lh_table_free(JC_OBJECT(jso_base)->c_object);
	json_object_generic_delete(jso_base);
}

struct json_object *doca_third_party_json_object_new_object(void)
{
	struct json_object_object *jso = JSON_OBJECT_NEW(object);
	if (!jso)
		return NULL;
	jso->c_object =
	    doca_third_party_lh_kchar_table_new(JSON_OBJECT_DEF_HASH_ENTRIES, &json_object_lh_entry_free);
	if (!jso->c_object)
	{
...
struct lh_table *doca_third_party_json_object_get_object(const struct json_object *jso)
{
	if (!jso)
		return NULL;
	switch (jso->o_type)
...
int doca_third_party_json_object_object_add_ex(struct json_object *jso, const char *const key,
                              struct json_object *const val, const unsigned opts)
{
	...
	existing_entry =
	    (opts & JSON_C_OBJECT_ADD_KEY_IS_NEW)
	        ? NULL
	        : doca_third_party_lh_table_lookup_entry_w_hash(JC_OBJECT(jso)->c_object, (const void *)key, hash);
	...
	if (!existing_entry)
	{
		...
		if (k == NULL)
			return -1;
		return doca_third_party_lh_table_insert_w_hash(JC_OBJECT(jso)->c_object, k, val, hash, opts);
	}
	existing_value = (json_object *)lh_entry_v(existing_entry);
	if (existing_value)
		doca_third_party_json_object_put(existing_value);
	lh_entry_set_val(existing_entry, val);
	return 0;
}

int doca_third_party_json_object_object_add(struct json_object *jso, const char *key, struct json_object *val)
{
	return doca_third_party_json_object_object_add_ex(jso, key, val, 0);
}

int doca_third_party_json_object_object_length(const struct json_object *jso)
{
	assert(doca_third_party_json_object_get_type(jso) == json_type_object);
	return doca_third_party_lh_table_length(JC_OBJECT_C(jso)->c_object);
}

size_t doca_third_party_json_c_object_sizeof(void)
{
	return sizeof(struct json_object);
}

struct json_object *doca_third_party_json_object_object_get(const struct json_object *jso, const char *key)
{
	struct json_object *result = NULL;
	doca_third_party_json_object_object_get_ex(jso, key, &result);
	return result;
}

json_bool doca_third_party_json_object_object_get_ex(const struct json_object *jso, const char *key,
                                    struct json_object **value)
{
	...
	{
	case json_type_object:
		return doca_third_party_lh_table_lookup_ex(JC_OBJECT_C(jso)->c_object, (const void *)key,
		                          (void **)value);
	default:
		if (value != NULL)
			*value = NULL;
...
void doca_third_party_json_object_object_del(struct json_object *jso, const char *key)
{
	assert(doca_third_party_json_object_get_type(jso) == json_type_object);
	doca_third_party_lh_table_delete(JC_OBJECT(jso)->c_object, key);
}
...
struct json_object *doca_third_party_json_object_new_boolean(json_bool b)
{
...
json_bool doca_third_party_json_object_get_boolean(const struct json_object *jso)
{
	if (!jso)
		return 0;
	switch (jso->o_type)
...
int doca_third_party_json_object_set_boolean(struct json_object *jso, json_bool new_value)
{
                                          ...
                                          int flags)
{
	...
	return doca_third_party_printbuf_memappend(pb, sbuf, strlen(sbuf));
}

struct json_object *doca_third_party_json_object_new_int(int32_t i)
{
	return doca_third_party_json_object_new_int64(i);
}

int32_t doca_third_party_json_object_get_int(const struct json_object *jso)
{
	int64_t cint64 = 0;
	double cdouble;
	...
	else if (o_type == json_type_string)
	{
		 ...
		 */
		if (doca_third_party_json_parse_int64(get_string_component(jso), &cint64) != 0)
...
int doca_third_party_json_object_set_int(struct json_object *jso, int new_value)
{
	return doca_third_party_json_object_set_int64(jso, (int64_t)new_value);
}

struct json_object *doca_third_party_json_object_new_int64(int64_t i)
{
...
struct json_object *doca_third_party_json_object_new_uint64(uint64_t i)
{
	struct json_object_int *jso = JSON_OBJECT_NEW(int);
	if (!jso)
		return NULL;
...
int64_t doca_third_party_json_object_get_int64(const struct json_object *jso)
{
	int64_t cint;
	...
	case json_type_boolean: return JC_BOOL_C(jso)->c_boolean;
	case json_type_string:
		if (doca_third_party_json_parse_int64(get_string_component(jso), &cint) == 0)
			return cint;
		/* FALLTHRU */
	default: return 0;
	}
...
uint64_t doca_third_party_json_object_get_uint64(const struct json_object *jso)
{
	uint64_t cuint;
	...
	case json_type_boolean: return JC_BOOL_C(jso)->c_boolean;
	case json_type_string:
		if (doca_third_party_json_parse_uint64(get_string_component(jso), &cuint) == 0)
			return cuint;
		/* FALLTHRU */
	default: return 0;
	}
...
int doca_third_party_json_object_set_int64(struct json_object *jso, int64_t new_value)
{
...
int doca_third_party_json_object_set_uint64(struct json_object *jso, uint64_t new_value)
{
	if (!jso || jso->o_type != json_type_int)
		return 0;
...
int doca_third_party_json_object_int_inc(struct json_object *jso, int64_t val)
{
...
int doca_third_party_json_c_set_serialization_double_format(const char *double_format, int global_or_thread)
{
...
#else
		doca_third_party__json_c_set_last_err("json_c_set_serialization_double_format: not compiled "
	...
	{
		doca_third_party__json_c_set_last_err("json_c_set_serialization_double_format: invalid "
                                                    ...
                                                    int level, int flags, const char *format)
{
	...
	if (size >= (int)sizeof(buf))
		...
		size = sizeof(buf) - 1;
	doca_third_party_printbuf_memappend(pb, buf, size);
	return size;
}
...
static int json_object_double_to_json_string(struct json_object *jso, struct printbuf *pb, int level,
                                      int flags)
{
	return doca_third_party_json_object_double_to_json_string(jso, pb, level, flags);
}

int doca_third_party_json_object_double_to_json_string(struct json_object *jso, struct printbuf *pb, int level,
                                      int flags)
{
...
struct json_object *doca_third_party_json_object_new_double(double d)
{
...
struct json_object *doca_third_party_json_object_new_double_s(double d, const char *ds)
{
	char *new_ds;
	struct json_object *jso = doca_third_party_json_object_new_double(d);
	if (!jso)
		return NULL;
	...
	doca_third_party_json_object_set_serializer(jso, _json_object_userdata_to_json_string, new_ds,
...
static int _json_object_userdata_to_json_string(struct json_object *jso, struct printbuf *pb,
                                                int level, int flags)
{
	return doca_third_party_json_object_userdata_to_json_string(jso, pb, level, flags);
}

int doca_third_party_json_object_userdata_to_json_string(struct json_object *jso, struct printbuf *pb, int level,
                                        int flags)
{
	int userdata_len = strlen((const char *)jso->_userdata);
	doca_third_party_printbuf_memappend(pb, (const char *)jso->_userdata, userdata_len);
	return userdata_len;
}

void doca_third_party_json_object_free_userdata(struct json_object *jso, void *userdata)
{
	free(userdata);
}

double doca_third_party_json_object_get_double(const struct json_object *jso)
{
	double cdouble;
	char *errPtr = NULL;
...
int doca_third_party_json_object_set_double(struct json_object *jso, double new_value)
{
	...
	if (jso->_to_json_string == &_json_object_userdata_to_json_string)
		doca_third_party_json_object_set_serializer(jso, NULL, NULL, NULL);
	return 1;
}
...
struct json_object *doca_third_party_json_object_new_string(const char *s)
{
	return _json_object_new_string(s, strlen(s));
}

struct json_object *doca_third_party_json_object_new_string_len(const char *s, const int len)
{
	return _json_object_new_string(s, len);
}

const char *doca_third_party_json_object_get_string(struct json_object *jso)
{
	if (!jso)
		return NULL;
	switch (jso->o_type)
	...
	default: return doca_third_party_json_object_to_json_string(jso);
	}
}
...
int doca_third_party_json_object_get_string_len(const struct json_object *jso)
{
	if (!jso)
		return 0;
	switch (jso->o_type)
...
int doca_third_party_json_object_set_string(json_object *jso, const char *s)
{
	return _json_object_set_string_len(jso, s, strlen(s));
}

int doca_third_party_json_object_set_string_len(json_object *jso, const char *s, int len)
{
                                            ...
                                            int flags)
{
	...
	printbuf_strappend(pb, "[");
	for (ii = 0; ii < doca_third_party_json_object_array_length(jso); ii++)
	{
		...
		if (flags & JSON_C_TO_STRING_SPACED && !(flags & JSON_C_TO_STRING_PRETTY))
			printbuf_strappend(pb, " ");
		indent(pb, level + 1, flags);
		val = doca_third_party_json_object_array_get_idx(jso, ii);
		if (val == NULL) {
...
{
	doca_third_party_json_object_put((struct json_object *)data);
}

static void json_object_array_delete(struct json_object *jso)
{
	doca_third_party_array_list_free(JC_ARRAY(jso)->c_array);
	json_object_generic_delete(jso);
}

struct json_object *doca_third_party_json_object_new_array(void)
{
	return doca_third_party_json_object_new_array_ext(ARRAY_LIST_DEFAULT_SIZE);
}
struct json_object *doca_third_party_json_object_new_array_ext(int initial_size)
{
	struct json_object_array *jso = JSON_OBJECT_NEW(array);
	if (!jso)
		return NULL;
	jso->c_array = doca_third_party_array_list_new2(&json_object_array_entry_free, initial_size);
	if (jso->c_array == NULL)
	{
		free(jso);
		return NULL;
...
struct array_list *doca_third_party_json_object_get_array(const struct json_object *jso)
{
	if (!jso)
		return NULL;
	switch (jso->o_type)
...
void doca_third_party_json_object_array_sort(struct json_object *jso, int (*sort_fn)(const void *, const void *))
{
	assert(doca_third_party_json_object_get_type(jso) == json_type_array);
	doca_third_party_array_list_sort(JC_ARRAY(jso)->c_array, sort_fn);
}

struct json_object *doca_third_party_json_object_array_bsearch(const struct json_object *key,
                                              ...
                                              int (*sort_fn)(const void *, const void *))
{
	...
	assert(doca_third_party_json_object_get_type(jso) == json_type_array);
	result = (struct json_object **)doca_third_party_array_list_bsearch((const void **)(void *)&key,
...
size_t doca_third_party_json_object_array_length(const struct json_object *jso)
{
	assert(doca_third_party_json_object_get_type(jso) == json_type_array);
	return doca_third_party_array_list_length(JC_ARRAY_C(jso)->c_array);
}

int doca_third_party_json_object_array_add(struct json_object *jso, struct json_object *val)
{
	assert(doca_third_party_json_object_get_type(jso) == json_type_array);
	return doca_third_party_array_list_add(JC_ARRAY(jso)->c_array, val);
}

int doca_third_party_json_object_array_insert_idx(struct json_object *jso, size_t idx, struct json_object *val)
{
	assert(doca_third_party_json_object_get_type(jso) == json_type_array);
	return doca_third_party_array_list_insert_idx(JC_ARRAY(jso)->c_array, idx, val);
}

int doca_third_party_json_object_array_put_idx(struct json_object *jso, size_t idx, struct json_object *val)
{
	assert(doca_third_party_json_object_get_type(jso) == json_type_array);
	return doca_third_party_array_list_put_idx(JC_ARRAY(jso)->c_array, idx, val);
}

int doca_third_party_json_object_array_del_idx(struct json_object *jso, size_t idx, size_t count)
{
	assert(doca_third_party_json_object_get_type(jso) == json_type_array);
	return doca_third_party_array_list_del_idx(JC_ARRAY(jso)->c_array, idx, count);
}

struct json_object *doca_third_party_json_object_array_get_idx(const struct json_object *jso, size_t idx)
{
	assert(doca_third_party_json_object_get_type(jso) == json_type_array);
	return (struct json_object *)doca_third_party_array_list_get_idx(JC_ARRAY_C(jso)->c_array, idx);
}
...
static int json_array_equal(struct json_object *jso1, struct json_object *jso2)
{
	...
	len = doca_third_party_json_object_array_length(jso1);
	if (len != doca_third_party_json_object_array_length(jso2))
		return 0;

	for (i = 0; i < len; i++)
	{
		if (!doca_third_party_json_object_equal(doca_third_party_json_object_array_get_idx(jso1, i),
		                       doca_third_party_json_object_array_get_idx(jso2, i)))
			return 0;
	}
	return 1;
}

int doca_third_party_json_object_array_shrink(struct json_object *jso, int empty_slots)
{
	if (empty_slots < 0)
		json_abort("json_object_array_shrink called with negative empty_slots");
	return doca_third_party_array_list_shrink(JC_ARRAY(jso)->c_array, empty_slots);
}

struct json_object *doca_third_party_json_object_new_null(void)
{
	return NULL;
}
...
static int json_object_all_values_equal(struct json_object *jso1, struct json_object *jso2)
{
	...
	assert(doca_third_party_json_object_get_type(jso1) == json_type_object);
	assert(doca_third_party_json_object_get_type(jso2) == json_type_object);
	/* Iterate over jso1 keys and see if they exist and are equal in jso2 */
	doca_third_party_json_object_object_foreachC(jso1, iter)
	{
		if (!doca_third_party_lh_table_lookup_ex(JC_OBJECT(jso2)->c_object, (void *)iter.key,
		                        (void **)(void *)&sub))
			return 0;
		if (!doca_third_party_json_object_equal(iter.val, sub))
			return 0;
	}

	/* Iterate over jso2 keys to see if any exist that are not in jso1 */
	doca_third_party_json_object_object_foreachC(jso2, iter)
	{
		if (!doca_third_party_lh_table_lookup_ex(JC_OBJECT(jso1)->c_object, (void *)iter.key,
		                        (void **)(void *)&sub))
			return 0;
	}
...
int doca_third_party_json_object_equal(struct json_object *jso1, struct json_object *jso2)
{
	if (jso1 == jso2)
		return 1;
...
static int json_object_copy_serializer_data(struct json_object *src, struct json_object *dst)
{
	...
	if (dst->_to_json_string == doca_third_party_json_object_userdata_to_json_string ||
	...
	{
		doca_third_party__json_c_set_last_err(
 ...
 */
int doca_third_party_json_c_shallow_copy_default(json_object *src, json_object *parent, const char *key,
                                size_t index, json_object **dst)
{
	switch (src->o_type)
	{
	case json_type_boolean: *dst = doca_third_party_json_object_new_boolean(JC_BOOL(src)->c_boolean); break;

	case json_type_double: *dst = doca_third_party_json_object_new_double(JC_DOUBLE(src)->c_double); break;

	case json_type_int:
		switch (JC_INT(src)->cint_type)
		{
		case json_object_int_type_int64:
			*dst = doca_third_party_json_object_new_int64(JC_INT(src)->cint.c_int64);
			break;
		case json_object_int_type_uint64:
			*dst = doca_third_party_json_object_new_uint64(JC_INT(src)->cint.c_uint64);
			break;
		default: json_abort("invalid cint_type");
		}
		break;
	...
	case json_type_string:
		*dst = doca_third_party_json_object_new_string_len(get_string_component(src),
		                                  _json_object_get_string_len(JC_STRING(src)));
		break;

	case json_type_object: *dst = doca_third_party_json_object_new_object(); break;

	case json_type_array: *dst = doca_third_party_json_object_new_array(); break;
                                           ...
                                           json_c_shallow_copy_fn *shallow_copy)
{
	...
	{
	case json_type_object:
		doca_third_party_json_object_object_foreachC(src, iter)
		{
			...
			if (doca_third_party_json_object_object_add(*dst, iter.key, jso) < 0)
			{
				doca_third_party_json_object_put(jso);
				return -1;
			}
	...
	case json_type_array:
		src_array_len = doca_third_party_json_object_array_length(src);
		for (ii = 0; ii < src_array_len; ii++)
		{
			struct json_object *jso = NULL;
			struct json_object *jso1 = doca_third_party_json_object_array_get_idx(src, ii);
			/* This handles the `json_type_null` case */
			if (!jso1)
				jso = NULL;
			...
			if (doca_third_party_json_object_array_add(*dst, jso) < 0)
			{
				doca_third_party_json_object_put(jso);
				return -1;
			}
...
int doca_third_party_json_object_deep_copy(struct json_object *src, struct json_object **dst,
                          json_c_shallow_copy_fn *shallow_copy)
{
	int rc;
	...
	if (shallow_copy == NULL)
		shallow_copy = doca_third_party_json_c_shallow_copy_default;

	rc = json_object_deep_copy_recursive(src, NULL, NULL, UINT_MAX, dst, shallow_copy);
	if (rc < 0)
	{
		doca_third_party_json_object_put(*dst);
		*dst = NULL;
	}
