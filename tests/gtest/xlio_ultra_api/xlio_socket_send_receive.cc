...
public:
    virtual void SetUp()
    {
        ...
        memset(&send_attr, 0, sizeof(send_attr));
        send_attr.userdata_op = 0x1; // Init to a magic number to trigger completion callbacks
    };
    ...
    static void socket_event_cb_nosend(xlio_socket_t sock, uintptr_t userdata_sq, int event,
                                       int value)
    {
    ...
    static void socket_event_cb_full_sq(xlio_socket_t sock, uintptr_t userdata_sq, int event,
                                        int value)
    {
        UNREFERENCED_PARAMETER(userdata_sq);
        UNREFERENCED_PARAMETER(value);
        if (event == XLIO_SOCKET_EVENT_ESTABLISHED) {
            pd = xlio_api->xlio_socket_get_pd(sock);
            ASSERT_TRUE(pd != NULL);
            mr_buf = ibv_reg_mr(pd, sndbuf, full_sq_segment_size, IBV_ACCESS_LOCAL_WRITE);
            ASSERT_TRUE(mr_buf != NULL);
            ...
            for (auto temp_msg = init_msgs; temp_msg; --temp_msg) {
                ASSERT_EQ(
                    0, xlio_api->xlio_socket_send(sock, sndbuf, full_sq_segment_size, &send_attr));
            }
    ...
    static void socket_rx_cb_nocheck(xlio_socket_t sock, uintptr_t userdata_sq, void *data,
    ...
    static void socket_accept_cb_send(xlio_socket_t sock, xlio_socket_t parent_sock,
                                      uintptr_t parent_userdata)
    {
        UNREFERENCED_PARAMETER(sock);
        UNREFERENCED_PARAMETER(parent_sock);
...
/**
 * @test ultra_api_socket_send_receive.ti_1
 * @brief
 *    Create TCP socket/connect/send(initiator)/receive(target)
 * @details
 */
TEST_F(ultra_api_socket_send_receive, ti_1)
{
    int rc;
    int pid = fork();
...
/**
 * @test ultra_api_socket_send_receive.ti_2
 * @brief
 *    Create TCP socket/connect/send(target)/receive(initiator)
 * @details
 */
TEST_F(ultra_api_socket_send_receive, ti_2)
{
    int rc;
    int pid = fork();
    ultra_api_base::SetUp();
    ...
    base_create_poll_group(&group, &socket_event_cb_nosend, &socket_comp_cb, &socket_rx_cb,
                           &socket_accept_cb_send);
    xlio_socket_attr sattr = {
        .flags = 0,
        .domain = server_addr.addr.sa_family,
        .group = group,
...
/**
 * @test ultra_api_socket_send_receive.ti_3
 * @brief
 *    Create TCP socket/connect/send(initiator)/receive(target) - full SQ
 * @details
 */
TEST_F(ultra_api_socket_send_receive, ti_3)
{
    int rc;
    int pid = fork();
    ultra_api_base::SetUp();
    ...
    base_create_poll_group(&group, &socket_event_cb_full_sq, &socket_comp_cb, &socket_rx_cb_nocheck,
                           &socket_accept_cb);
    xlio_socket_attr sattr = {
        .flags = 0,
        .domain = server_addr.addr.sa_family,
    ...
    if (pid == 0) {
        ...
        while (num_wqe_to_send_temp-- > 0) {
            ASSERT_EQ(0,
                      xlio_api->xlio_socket_send(sock, sndbuf, full_sq_segment_size, &send_attr));
            xlio_api->xlio_socket_flush(sock);
        }
