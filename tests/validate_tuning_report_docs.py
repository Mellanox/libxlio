...
# Scenario snippets include a few dynamic/context fields that are valid report
# output but cannot be matched one-to-one against static printer field names.
SCENARIO_FIELD_EXEMPTIONS = [
    r"buffer_pool_\w+_(size|alloc_failures)",
    r"hugepages_\d+kB_(total|free)",
    r"nic_device",
    r"ring_total_\w+",
]
...
def extract_field_names_from_code(cpp_path: Path) -> set[str]:
    ...
    content = cpp_path.read_text()
    fields = set()
    for m in re.finditer(r'fprintf\(f,\s*"([^":]+):', content):
        field = m.group(1)
        if not re.match(r"[a-z]", field):
            continue
        for printf_token, template_name in FIELD_NAME_TEMPLATES.items():
            field = field.replace(printf_token, template_name)
        fields.add(field)
    return fields


def is_known_dynamic_or_context_field(field: str) -> bool:
    """Return whether a scenario-only field is known dynamic or contextual."""
    return any(re.fullmatch(pat, field) for pat in SCENARIO_FIELD_EXEMPTIONS)
...
def extract_field_names_from_reference(md_path: Path) -> set[str]:
    ...
    section = match.group(1)
    fields = set()
    for m in re.finditer(r"\| `([a-z][A-Za-z0-9_]+(?:<[^>]+>)?[A-Za-z0-9_]*)` \|", section):
...
def validate_guide(
    ...
    for field in sorted(scenario_fields):
        if field in code_fields:
            continue
        if is_known_dynamic_or_context_field(field):
            continue
