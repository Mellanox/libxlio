...
void ring_bond::restart()
{
    ...
    int ret = 0;
    ret = request_notification(CQT_RX);
    if (ret < 0) {
        ring_logdbg("failed arming cq_mgr_rx (errno=%d %m)", errno);
    }
    ret = request_notification(CQT_TX);
    if (ret < 0) {
...
int ring_bond::request_notification(cq_type_t cq_type)
{
    int ret = 0;
    int temp;
    ...
    for (uint32_t i = 0; i < (*bond_rings).size(); i++) {
        if ((*bond_rings)[i]->is_up()) {
            temp = (*bond_rings)[i]->request_notification(cq_type);
            if (temp < 0) {
                ret = temp;
                break;
            } else {
