...
int cq_mgr_rx_regrq::drain_and_proccess(uintptr_t *p_recycle_buffers_last_wr_id /*=NULL*/)
{
    ...
    uint32_t ret_total = 0;
    /* RX-batch timestamp is refreshed once, on the first real buffer in this
     * drain, and cleared on exit by the guard's destructor (so a later read
     * without a refresh sees the detectable 0 sentinel, not a stale value). */
    ...
    while ((m_n_sysvar_progress_engine_wce_max > ret_total) || p_recycle_buffers_last_wr_id) {
        ...
        if (!buff) {
             ...
             * drain never refreshed (no buffer processed, but GRO streams
             * carried over from a prior pass). */
        ...
        /* buff is non-null here (regrq has no filler CQE), so this refreshes
         * once on the first processed buffer. */
