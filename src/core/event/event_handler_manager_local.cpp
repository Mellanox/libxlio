...
void event_handler_manager_local::do_tasks()
{
    ...
    // Debug: log ONLY first call to see inherited state
    static thread_local bool first_call = true;
    if (first_call) {
        __log_err("[FORK_FIX] FIRST do_tasks() call: PID=%d elapsed_ms=%ld", getpid(), elapsed_ms);
        first_call = false;
    }

    if (likely(safe_mce_sys().tcp_timer_resolution_msec > elapsed_ms)) {
        return;
    }
...
void event_handler_manager_local::reset_after_fork()
{
    // After fork(), child process inherits parent's m_last_run_time timestamp.
    // This causes rate-limiting to prevent timers from running for up to 100ms.
    // Reset timestamps so timers run immediately in the child process.
    __log_err("[FORK_FIX] reset_after_fork() called in PID=%d", getpid());
    m_last_run_time = steady_clock::time_point(); // Reset to epoch
    m_last_taken_time = steady_clock::now();
    __log_err("[FORK_FIX] reset complete, m_last_run_time set to epoch");
}
