...
parameter_descriptor::parameter_descriptor(const parameter_descriptor &pd)
    : m_default_value(pd.m_default_value), m_constraints(pd.m_constraints),
      m_string_mapping(pd.m_string_mapping) {}

void parameter_descriptor::add_string_mapping(
    const std::string &str, const std::experimental::any &val) {
...
parameter_descriptor::get_value(const std::experimental::any &val) const {
  if (val.type() == typeid(std::string)) {
    auto it =
        m_string_mapping.find(std::experimental::any_cast<std::string>(val));
...
bool parameter_descriptor::validate_constraints(
    const std::experimental::any &val) const {
