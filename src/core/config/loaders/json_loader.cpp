...
static std::experimental::any to_any_value(json_object *obj)
{
    ...
    if (converter == type_converters.end()) {
        ...
        throw_xlio_exception(ERROR_STRING("Unsupported JSON type: " + std::to_string(type)));
    }
    ...
    : m_file_path(file_path)
{
    ...
    if (!ifs.good()) {
        throw_xlio_exception(ERROR_STRING("Cannot open file: " + m_file_path));
    }
}
...
std::map<std::string, std::experimental::any> json_loader::load_all() &
{
    ...
    if (!raw_obj) {
        throw_xlio_exception(ERROR_STRING("Failed to parse JSON file: " + m_file_path));
    }

    json_object_handle root_obj(raw_obj);

    if (json_object_get_type(root_obj.get()) != json_type_object) {
        throw_xlio_exception(ERROR_STRING("Top-level JSON is not an object: " + m_file_path));
    }
