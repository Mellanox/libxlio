            ...
            do {
                ...
                if (unlikely(err != ERR_OK)) {
                    if (unlikely(err == ERR_CONN)) { // happens when remote drops during big write
                        ...
                        shutdown(SHUT_WR);
                        return tcp_tx_handle_partial_send_and_unlock(total_tx, EPIPE, is_dummy, errno_tmp);
                    }
                    ...
                    if (unlikely(g_b_exit)) {
                        return tcp_tx_handle_partial_send_and_unlock(total_tx, EINTR, is_dummy, errno_tmp);
                    }
