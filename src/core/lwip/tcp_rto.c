...
s32_t tcp_rto_from_estimator_us(s32_t sa_us, s32_t sv_us)
{
     ...
     * sv_us ~ 4*RTTVAR). Applying the floor to the variance allowance reserves
     * it beyond measured SRTT; at idle (SRTT ~ 0), RTO is approximately the
     * floor. */
...
void tcp_rtt_estimator_update_us(struct tcp_pcb *pcb, int64_t raw_sample_us)
{
    ...
    if (pcb->sa_us == 0) {
        const s32_t seed_sv_us = pcb->sv_us;

        /* Initialize the VJ estimator directly. A variance seed above the
         * initial RTO denotes the SYN-loss fallback and remains in force. */
        pcb->sa_us = tcp_estimator_clamp_i32(m_us << 3);
        if (seed_sv_us > TCP_RTO_INITIAL_US) {
            pcb->sv_us = seed_sv_us;
        } else {
...
{
    uint64_t tick_floor_us = (uint64_t)TCP_OOSEQ_RTO_MIN_TICKS * slow_interval_us;
    uint64_t rto_us_pos = pcb->rto_us > 0 ? (uint64_t)pcb->rto_us : 0;
    uint64_t ooseq_rto_us = rto_us_pos > tick_floor_us ? rto_us_pos : tick_floor_us;
...
{
    pcb->rttest_us = 0;
    /* rttest_us == 0 makes rtseq inactive; clear it for truthful diagnostics. */
    pcb->rtseq = 0;
    pcb->sa_us = 0;
